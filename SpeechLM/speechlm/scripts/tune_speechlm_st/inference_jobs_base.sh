...
for lang in ar de ca tr; do
    ...
    for seed in seed2 seed3; do
        model=/valle/v-ziqzhang/data/speechulm/exp/finetune_covost/base_speechlmp_32gpu_1accum/legacy_en${lang}_${seed}_bz3.2m_lr1e-4/checkpoint_best.pt
        for beam_size in 5 10; do 
            bash speechlm/scripts/tune_speechlm_st/inference_base.sh $model $data $lang $gen_set $beam_size
        done
    done
done
